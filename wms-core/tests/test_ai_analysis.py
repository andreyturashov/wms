...
class TestAITaskAnalysis:
    async def test_creating_task_triggers_ai_comments(self, authed_client: AsyncClient):
        ...
        assert author_names == {"Executor Agent", "Thinker Agent"}
        for c in comments:
    ...
    async def test_executor_comment_contains_plan(self, authed_client: AsyncClient):
        ...
        executor_comment = next(c for c in comments if c["author_name"] == "Executor Agent")
        assert "Execution Plan" in executor_comment["content"]
        assert "Estimated Total Effort" in executor_comment["content"]
    ...
    async def test_thinker_comment_contains_analysis(self, authed_client: AsyncClient):
        ...
        thinker_comment = next(c for c in comments if c["author_name"] == "Thinker Agent")
        assert "Problem Analysis" in thinker_comment["content"]
        assert "Proposed Approaches" in thinker_comment["content"]
